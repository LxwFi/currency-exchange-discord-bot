...
class CurrencyConverter {
    ...
    amount(currencyAmount) {
        ...
        if (isNaN(currencyAmount))
            throw new Error("currency amount should be a number")
    ...
    rates() {
        if (this.currencyFrom === this.currencyTo)
            ...
            return got(`https://www.google.com/search?q=100+${this.currencyFrom}+to+${this.currencyTo}`)
                .then((html) => { return cheerio.load(html.body) })
                .then(($) => { return $(".iBp4i").text().split(" ")[0] })
                .then((rates) => {
                    if (rates.includes(","))
                        rates = rates.replace(",", ".")
                    return Math.round(parseFloat(rates / 100) * 1e5) / 1e5
                })
    }
    ...
    convert(currencyAmount) {
        ...
        return this.rates().then((rates) => {
            ...
            if (this.convertedValue < 1) {
                return this.convertedValue
            } else {
                return Math.round(this.convertedValue * 1e2) / 1e2
            }
        })
    }
